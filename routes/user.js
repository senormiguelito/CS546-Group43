...
router
  ...
  .get(async (req, res) => {
    try {
      ...
      res.status(200).render("login", { title: "Login" });    // what is this for? Don't need in both places, pick one
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      if (loginDetails.authentication) {
        req.session.user = {
//          userID: loginDetails.userID,  --> I will create a string userID object generated by mongo, then add that to the user Object, better to use that here 
        ...
        res
          .status(400)
          .render("login", { Error: "sorry, user not authenticated" });
      }
    } catch (e) {
      // console.log(e);
      res.status(400).render("login", { Error: e });
