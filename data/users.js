...
export const getUser = async (id) => {
  ...
  if (typeof id !== "string" || id.trim() === "") throw new Error("getUser: ID must be of type string and non empty");
  id = id.trim();
...
export const remove = async (id) => {
  ...
  if (typeof id !== "string" || id.trim() === "") throw new Error("Remove: ID must be a non-empty string");
  id = id.trim();
  if (!Object.isValid(id)) throw new Error("Remove: invalid Object ID");

  const userCollection = await users();
  const removalInfo = await userCollection.findOneAndDelete({ _id: new ObjectId(id) });

  if (removalInfo.deletedCount === 0) throw new Error(`Remove: could not delete user with ID of ${id}`);
...
export const update = async (
  ...
  if (!firstName || !lastName || !emailAddress || !password || !phoneNumber || !location_city || !location_state || !location_zip_code || !categories || !bio) {
    throw new Error("every field must be provided");
  }
  if (typeof firstName !== "string") throw new Error("first name must be of type string");
  if (typeof lastName !== "string") throw new Error("last name must be of type string");
  ...
  // bio:
  if (typeof bio !== 'string') throw new Error("Update: bio must be of type string");
  if (bio.length > 5000) throw new Error("Update: bio can not exceed 5000 characters");
  if (bio.trim() === "") throw new Error("Update: bio must not be empty");


  // categories
  if (!Array.isArray(categories)) throw new Error("Update: categories must be an array");
  if (categories.length < 1) throw new Error("Update: you must supply at least 1 category");
  for (let i in categories) {
    if (typeof categories[i] !== "string" || categories[i].trim() === "") throw new Error("Update: each category must be a non-empty string");
    categories[i] = categories[i].trim();
    for (let j in categories[i]) {
      if (typeof categories[i][j] === 'number') throw new Error("Update: invalid category response");
    }
  }

  const userCollection = await users();
  const updateUserInfo = {  // only include in here the stuff that makes sense for the user to update
    firstName: firstName,
    lastName: lastName,
    emailAddress: emailAddress,
    password: hashPassword,
  ...
  const userUpdate = await userCollection.updateOne({ _id: new ObjectId(id) }, { $set: updateUserInfo });
